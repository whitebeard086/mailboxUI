...
import { setMessage, setMessageData, setMessages, useAppDispatch, useAppSelector } from '@/store';
import { useGetMessages } from '../home/utils/hooks';
...
const Inbox = () => {
    ...
    useEffect(() => {
        if (status === 'success') {
            ...
            const newMessages = data?.data?.filter(message => !existingMessages.includes(message.id)) ?? []
        ...
        <div className='max-w-5xl mx-auto lg:px-6'>
            <div className='flex items-center justify-between mb-4'>
                <div className='flex flex-col'>
                    ...
                    <p className='font-semibold text-gray-500'>
                        {user?.messagesCount} Messages | {user?.unreadMessagesCount} Unread
                    </p>
                </div>
            ...
            <div className='max-h-[70svh] overflow-auto'>
                <Table>
                    ...
                    <TableBody >
                        {messages?.map((message) => (
                            <TableRow ref={message.id === lastMessage?.id ? ref : null} key={message.id} onClick={() => onViewMessage(message)} className={cn(
                                'hover:cursor-pointer',
                                !message.isRead && 'font-bold'
                            )}>
                                <TableCell className='relative'>
                                    {message.sender.name}
                                    {!message.isRead && <span className={cn('absolute top-3 h-2 w-2 rounded-full bg-red-500')} />}
                                </TableCell>
                                <TableCell>{message.preview}</TableCell>
                                <TableCell className='text-right'>
                                    {dayjs(message.createdAt).format('Do MMM, YYYY')}
                                </TableCell>
                            </TableRow>
                        ))}
                    </TableBody>
                ...
                <div className='flex justify-center mt-4'>
                    {isLastPage ? (
                        <p className='text-gray-500 text-center'>No more messages</p>
                    ) : (
